...
function TableDetailContent() {
  ...
  const { userData, companyId, branchId, currentUser } = useAuth();
  ...
  const [pendingPaymentItems, setPendingPaymentItems] = useState<Array<{
    menuId: string;
    menuName: string;
    totalQuantity: number;
    indices: number[];
  }>>([]);
  const [currentPaymentItemIndex, setCurrentPaymentItemIndex] = useState<number>(0);
  const [selectedQuantities, setSelectedQuantities] = useState<Map<string, number>>(new Map());
  
  // İptal için state'ler (ödeme gibi)
  const [pendingCancelItems, setPendingCancelItems] = useState<Array<{
    menuId: string;
    menuName: string;
    totalQuantity: number;
    indices: number[];
  }>>([]);
  const [currentCancelItemIndex, setCurrentCancelItemIndex] = useState<number>(0);
  const [selectedCancelQuantities, setSelectedCancelQuantities] = useState<Map<string, number>>(new Map());
  
  // Taşıma için state'ler (ödeme gibi)
  const [pendingMoveItems, setPendingMoveItems] = useState<Array<{
    menuId: string;
    menuName: string;
    totalQuantity: number;
    indices: number[];
  ...
  const [selectedMoveQuantities, setSelectedMoveQuantities] = useState<Map<string, number>>(new Map());
  ...
  const addToCartWithNote = useCallback((menu: Menu, note?: string) => {
    setCart((prev) => {
  ...
  // Ürün ekleme butonuna tıklandığında direkt sepete ekle (not olmadan)
      ...
      return prev.map((i) =>
        i.cartItemId === cartItemId
          ? {
              ...
              subtotal: newQuantity * i.menuPrice,
            }
          : i
      );
    });
  ...
  const handleSendOrder = useCallback(async () => {
    ...
    try {
      ...
      if (updatedOrder) {
        ...
        const effectiveBranchIdForHistory = branchId || userData?.assignedBranchId;
        try {
      ...
      // İşlem tamamlandı, ana sayfaya yönlendir
      navigateToHome();
    } catch (error) {
  ...
  }, [userData, cart, order, notes, currentTable, navigate]);
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      const isPartialPayment = selectedItems.size > 0;
      ...
      if (isPartialPayment) {
        ...
        if (pendingPaymentItems.length > 0) {
          ...
          pendingPaymentItems.forEach((paymentItem) => {
            ...
            const selectedQty = selectedQuantities.get(paymentItem.menuId) || paymentItem.totalQuantity;
            ...
            for (const index of paymentItem.indices) {
              ...
              const paidQuantity = item.quantity <= remainingQuantity 
                ? item.quantity 
                : remainingQuantity;
      ...
      if (isPartialPayment && selectedItems.size > 0) {
        ...
        // State'i hemen güncelle (ödenen ürünlerin görünmemesi için)
        setOrder({
          ...order,
          items: updatedItems,
          subtotal: newSubtotal,
      ...
      if (effectiveCompanyId) {
        try {
          if (isPartialPayment && paidItems) {
            // Kısmi ödeme
            const paymentMethodName = paymentMethod === "cash" ? "Nakit" : paymentMethod === "card" ? "Kart" : "Yemek Kartı";
            await addTableHistory(
              effectiveCompanyId,
              currentTable.id!,
              currentTable.tableNumber,
      ...
      // Kurye ataması yapıldıysa
      if (selectedCourierId && updatedOrder) {
        const courier = couriers.find((c) => c.id === selectedCourierId);
        if (courier) {
          const effectiveCompanyId = companyId || userData?.companyId;
          const effectiveBranchId = branchId || userData?.assignedBranchId;
          const packageCountNum = parseInt(packageCount) || 1;
      ...
      // Eğer TAM ödeme alındıysa (ve partial ödeme değilse) veya tüm ürünler kaldırıldıysa siparişi kapat ve anasayfaya gönder
      ...
      if (
        ...
        // Alert vermeden direkt anasayfaya gönder (search params'ı koru)
        navigateToHome();
        return;
      }
  ...
  }, [order, selectedItem, currentTable, navigate, companyId, branchId, userData]);
  ...
  const handleCancelSelectedItems = useCallback(async () => {
    ...
    try {
      ...
      if (effectiveCompanyId) {
        try {
          const canceledItemsForHistory = order.items.filter((_, index) => selectedItems.has(index));
  ...
  }, [order, selectedItems, currentTable, navigate, companyId, branchId, userData]);
  ...
  const handleMoveSelectedItems = useCallback(
    ...
    try {
      ...
      if (pendingMoveItems.length > 0) {
        ...
        pendingMoveItems.forEach((moveItem) => {
          // Eğer bu ürün için miktar seçildiyse kullan, yoksa tüm miktarı kullan
          const selectedQty = selectedMoveQuantities.get(moveItem.menuId) || moveItem.totalQuantity;
          let remainingQuantity = selectedQty;
        ...
        let updatedItems = order.items.map((it, idx) => {
    ...
    [order, selectedItems, currentTable, userData, navigate, pendingMoveItems, selectedMoveQuantities]
  );
  ...
  return (
    <div className="h-[100dvh] bg-gray-50 dark:bg-gray-900 flex flex-col lg:flex-row overflow-hidden">
      {/* Header - Mobile */}
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <div className="hidden lg:flex lg:w-80 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex-col h-full overflow-hidden relative">
          {/* Detay Paneli - Sağdan Kayarak */}
          <div
            ...
            showOrderDetailModal ? "translate-x-0" : "translate-x-full"
            }`}
          >
            <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
              <div className="flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <div className="space-y-3">
                ...
                {/* Tüm Güncellemeler - Birleştirilmiş ve Sıralanmış */}
                {order &&
                  (() => {
                    // Aktif ürünleri işle
                  ...
                  if (group.length > 0) {
                      ...
                      processedItems.push({
                        item: {
                          ...
                          notes: combinedNotes || undefined,
                              ...
                              movedFromTableNumber: firstItem.movedFromTableNumber,
                    ...
                    if (order.canceledItems) {
                      order.canceledItems.forEach((canceledItem) => {
                        ...
                        if (itemAddedAt) {
                          // Aktif ürünlerden aynı zamanda eklenenleri bul (gruplanmamış halini kullan)
                          const sameTimeActiveItems = order.items.filter((activeItem) => {
                            if (activeItem.menuId !== canceledItem.menuId) return false;
                            ...
                            return Math.abs(activeAddedAt - itemAddedAt) < 1000;
                          });
                          
                          // İptal edilen ürünlerden aynı zamanda eklenenleri bul
                          const sameTimeCanceledItems = (order.canceledItems || []).filter((ci) => {
                            if (ci.menuId !== canceledItem.menuId) return false;
                          ...
                          // Toplam orijinal miktarı hesapla (aktif + iptal edilen)
                          const activeQuantity = sameTimeActiveItems.reduce(
                            (sum, item) => sum + item.quantity,
                            0
                          );
                          const canceledQuantity = sameTimeCanceledItems.reduce(
                            (sum, item) => sum + item.quantity,
                            0
                          );
                          originalQuantity = activeQuantity + canceledQuantity;
                        }
                    ...
                    const groupedMovedItems: { [key: string]: OrderItem[] } =
                      {};
                    if (order.movedItems) {
                    ...
                    Object.values(groupedMovedItems).forEach((group) => {
                      if (group.length > 0) {
                        ...
                        const latestMovedAt = group.reduce<Date | undefined>(
                          (latest, item) => {
                        ...
                        const earliestAddedAt = group.reduce<Date | undefined>(
                          (earliest, item) => {
                    ...
                    <div className="space-y-3">
                        ...
                        return (
                          <div
                                ...
                                <div className="flex items-start justify-between mb-2">
                                  ...
                                  <span className="font-bold text-sm text-gray-900 dark:text-white">
                                    ₺{item.subtotal.toFixed(2)}
                                      ...
                                      {(item.movedFromTableNumber || item.movedFromTableId) ? (
                                        <div className="flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 pt-2 border-t border-gray-200 dark:border-gray-600">
                                          ...
                                          <span>
                                            {item.movedAt ? getTimeAgo(item.movedAt) : update.addedAt ? getTimeAgo(update.addedAt) : ""} Masa {item.movedFromTableNumber || "Bilinmeyen"}'den Masa {currentTable.tableNumber}'ye taşındı
                                          </span>
                                        </div>
                                      ) : update.addedAt ? (
                            ...
                            const displayQuantity = update.originalQuantity || item.quantity;
                            return (
                              <div
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-1 text-xs text-red-600 dark:text-red-400 pt-2 border-t border-red-200 dark:border-red-800">
                                ...
                                <div className="flex items-center gap-1">
                                  <Clock className="h-3 w-3" />
                                      <span>
                                        {getTimeAgo(update.addedAt)} {displayQuantity} adet eklendi
                                      </span>
                                </div>
                              )}
                                  {update.canceledAt && (
                                <div className="flex items-center gap-1">
                                  <Clock className="h-3 w-3" />
                                      <span>
                                        {getTimeAgo(update.canceledAt)} iptal
                                        edildi
                                      </span>
                                </div>
                                  )}
                                  ...
                                  {update.movedAt && (
                                    <div className="flex items-center gap-1">
                                      ...
                                      <span>
                                        ...
                                        {update.movedFromTableNumber && update.movedToTableNumber
                          ...
                          >
                            ...
                            {couriers.map((courier) => (
                              <option key={courier.id} value={courier.id}>
                                {courier.name} (₺{courier.pricePerPackage.toFixed(2)}/paket)
                              </option>
                            ))}
                          </select>
          ...
          <div
            className={`flex-1 flex flex-col transition-opacity duration-300 overflow-hidden ${
              showOrderDetailModal || showPaymentModal
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-3 space-y-3 flex-shrink-0">
            {order && order.items.length > 0 ? (
              // Aktif Sipariş İşlemleri
                ...
                {cart.length === 0 && (
                  <>
                    {/* Seçili Ürünler İçin İptal ve Taşı Butonları */}
                        ...
                        <Button
                          onClick={() => {
                            ...
                            if (itemsByMenuId.size > 1) {
                              ...
                              const itemsArray = Array.from(itemsByMenuId.values()).map((item) => {
                                const totalQuantity = item.indices.reduce(
                                  (sum, idx) => {
                                    const orderItem = order.items[idx];
                                    return orderItem ? sum + orderItem.quantity : sum;
                                  },
                                  0
                                );
                                return {
                            ...
                            if (itemsByMenuId.size === 1) {
                              const singleItem = Array.from(itemsByMenuId.values())[0];
                              const totalQuantity = singleItem.indices.reduce(
                                (sum, idx) => {
                          ...
                          onClick={() => {
                            if (selectedItems.size > 0) {
                              ...
                              if (itemsByMenuId.size > 1) {
                                ...
                                // İlk ürün için miktar seçim modalı aç
                                setPendingPaymentItems(itemsArray);
                                setCurrentPaymentItemIndex(0);
                                setSelectedItemForQuantity(itemsArray[0]);
                                setSelectedQuantity(1);
                                setQuantitySelectionAction("payment");
                                setShowQuantitySelectionModal(true);
                                return;
                              }
                              ...
                              if (itemsByMenuId.size === 1) {
                                ...
                                if (totalQuantity > 1) {
                                  // Aynı üründen birden fazla adet var, miktar seçim modalı aç
                                setSelectedItemForQuantity({
                                    menuId: singleItem.item.menuId,
                                    menuName: singleItem.item.menuName,
                                  totalQuantity: totalQuantity,
                              ...
                              // Tek ürün ama 1 adet ise direkt ödeme modalı aç
                            }
                            
                            // Aynı üründen birden fazla yoksa direkt ödeme modalını aç
                            let remaining: number;
                            if (selectedItems.size > 0) {
                              // Kısmi ödeme: Sadece seçili ürünlerin toplamı
                                  const selectedItemsArray =
                                    Array.from(selectedItems);
                                  const selectedItemsData = selectedItemsArray
                                    .map((index) => order.items[index])
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 p-3 space-y-3 bg-gray-50 dark:bg-gray-700/30 flex-shrink-0">
                ...
                {/* Send Button */}
                  <Button
                    onClick={handleSendOrder}
                    disabled={cart.length === 0 || isSendingOrder}
                    className="w-full h-10 text-sm font-medium text-white dark:text-white"
                  >
                    {isSendingOrder ? (
                      <>
                        <Loader2 className="h-3 w-3 mr-2 animate-spin" />
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full h-24 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                />
              </div>
              <div className="flex gap-2">
                <Button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div 
            key={`quantity-modal-${selectedItemForQuantity.menuId}-${
              quantitySelectionAction === "payment" ? currentPaymentItemIndex :
              quantitySelectionAction === "cancel" ? currentCancelItemIndex :
                ...
                <Button
                  onClick={async () => {
                    ...
                    if (quantitySelectionAction === "cancel") {
                      ...
                      if (pendingCancelItems.length > 0 && currentCancelItemIndex < pendingCancelItems.length - 1) {
                        // Bu ürünün miktarını kaydet (güncel map'i kullan)
                        const currentItem = pendingCancelItems[currentCancelItemIndex];
                        const updatedQuantities = new Map(selectedCancelQuantities);
                        updatedQuantities.set(currentItem.menuId, selectedQuantity);
                        ...
                        setSelectedItemForQuantity(pendingCancelItems[nextIndex]);
                        setSelectedQuantity(1);
                      ...
                      try {
                        ...
                        const allSelectedQuantities = new Map(selectedCancelQuantities);
                        if (pendingCancelItems.length > 0) {
                          // Son ürünün miktarını da ekle
                          const lastItem = pendingCancelItems[pendingCancelItems.length - 1];
                          allSelectedQuantities.set(lastItem.menuId, selectedQuantity);
                      ...
                      const indicesToCancel = new Set<number>();
                        ...
                        if (pendingCancelItems.length > 0) {
                          ...
                          pendingCancelItems.forEach((cancelItem) => {
                            const selectedQty = allSelectedQuantities.get(cancelItem.menuId) || cancelItem.totalQuantity;
                            let remainingQuantity = selectedQty;
                            ...
                            for (const index of cancelItem.indices) {
                              ...
                              const cancelQuantity = Math.min(remainingQuantity, item.quantity);
                      ...
                      const canceledItems = currentOrder.canceledItems || [];
                      ...
                      let updatedItems = currentOrder.items.map((it, idx) => {
                      ...
                      // Eğer birden fazla ürün için miktar seçiliyorsa, bir sonraki ürün için modal aç
                      if (pendingPaymentItems.length > 0 && currentPaymentItemIndex < pendingPaymentItems.length - 1) {
                        // Bu ürünün miktarını kaydet (güncel map'i kullan)
                        const currentItem = pendingPaymentItems[currentPaymentItemIndex];
                        ...
                        setSelectedItemForQuantity(pendingPaymentItems[nextIndex]);
                        setSelectedQuantity(1);
                        // Modal açık kalacak, sadece içeriği değişecek
                        return;
                      }
                      ...
                      // Tüm seçilen miktarları bir map'te topla (state güncellemesinden önce)
                      const allSelectedQuantities = new Map(selectedQuantities);
                      if (pendingPaymentItems.length > 0) {
                        // Son ürünün miktarını da ekle
                        const lastItem = pendingPaymentItems[pendingPaymentItems.length - 1];
                        allSelectedQuantities.set(lastItem.menuId, selectedQuantity);
                        ...
                        const firstQuantity = allSelectedQuantities.get(firstItem.menuId) || firstItem.totalQuantity;
                      ...
                      if (pendingPaymentItems.length > 0) {
                        ...
                        pendingPaymentItems.forEach((item) => {
                          const quantity = allSelectedQuantities.get(item.menuId) || item.totalQuantity;
                          ...
                          item.indices.forEach((index) => {
                            ...
                            if (orderItem.quantity <= remainingQty) {
                              ...
                              paymentTotal += partialQty * orderItem.menuPrice;
                              remainingQty = 0;
                            }
                          });
                        });
                      ...
                      // Eğer birden fazla ürün için miktar seçiliyorsa, bir sonraki ürün için modal aç
                      if (pendingMoveItems.length > 0 && currentMoveItemIndex < pendingMoveItems.length - 1) {
                        // Bu ürünün miktarını kaydet (güncel map'i kullan)
                        const currentItem = pendingMoveItems[currentMoveItemIndex];
                        const updatedQuantities = new Map(selectedMoveQuantities);
                        updatedQuantities.set(currentItem.menuId, selectedQuantity);
                        ...
                        setSelectedItemForQuantity(pendingMoveItems[nextIndex]);
                        setSelectedQuantity(1);
                        // Modal açık kalacak, sadece içeriği değişecek
                        return;
                      }
                      ...
                      // Tüm seçilen miktarları bir map'te topla (state güncellemesinden önce)
                      const allSelectedQuantities = new Map(selectedMoveQuantities);
                      if (pendingMoveItems.length > 0) {
                        // Son ürünün miktarını da ekle
                        const lastItem = pendingMoveItems[pendingMoveItems.length - 1];
                        allSelectedQuantities.set(lastItem.menuId, selectedQuantity);
