...
export function TablesManagementContent() {
  ...
  useEffect(() => {
    const loadTables = async () => {
      ...
      try {
        ...
        const tablesWithoutArea = uniqueTables.filter((t) => {
  ...
  const handleDeleteArea = async (areaName: string) => {
    if (
      !confirm(
        `"${areaName}" alanını ve içindeki tüm masaları silmek istediğinize emin misiniz?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (selectedArea === areaName && tablesData.length > 0) {
        const remainingAreas = Array.from(
          new Set(tablesData.map((t) => t.area))
        ).sort();
        ...
        {areas.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm mt-2">
              Yukarıdaki butona tıklayarak ilk alanınızı ekleyin
            </p>
          </div>
        ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6 mb-6">
          ...
          {tablesInArea.length === 0 ? (
            <div className="text-center py-6 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm mt-2">
                Yukarıdaki "Masa Ekle" butonuna tıklayın
              </p>
            </div>
          ) : (
